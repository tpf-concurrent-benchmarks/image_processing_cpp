...
void change_resolution(std::string path, int output_width, int output_height, std::string output_path) {
    ...
    stbir_resize_uint8_linear(input_image, input_width, input_height, 0,
                       output_image, output_width, output_height, 0, static_cast<stbir_pixel_layout>(output_channels));

    stbi_write_png(output_path.c_str(), output_width, output_height, output_channels, output_image, output_width * output_channels);
